// ── Get current user's UID for per-user storage ───────────────────────────────
...
export const getSnippets = () =>
  JSON.parse(localStorage.getItem(snippetsKey()) || '[]')
...
export function deleteSnippet(id) {
  ...
  localStorage.setItem(snippetsKey(), JSON.stringify(s))
  return s
}
...
export function toggleFavorite(id) {
  ...
  const i = snippets.findIndex(s => s.id === id)
  if (i > -1) {
    snippets[i].favorite = !snippets[i].favorite
    localStorage.setItem(snippetsKey(), JSON.stringify(snippets))
  }
  return snippets
}
...
export function seedSnippets() {
  // Only seed if this user has no snippets yet
  if (getSnippets().length > 0) return
  const examples = [
    {
      title: 'React Login Form',
      language: 'jsx',
      tags: ['react', 'form', 'auth'],
      description: 'Reusable login form with loading state',
    ...
    },
    {
      ...
      description: 'Basic REST route with error handling',
    ...
    },
    {
      ...
      description: 'Mongoose connect with error handling',
    ...
    },
    {
      ...
      description: 'Robust fetch wrapper with timeout',
